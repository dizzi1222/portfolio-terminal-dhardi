...
setInterval(() => {
  spinCert('cert-modal-spinner');
  const d = document.getElementById('cert-modal-date');
  if (d) d.textContent = new Date().toISOString().replace('T', ' ').slice(0, 19) + 'Z';
}, 400);
function advanceBar() {
  const el = document.getElementById('cert-bar');
  if (!el) return;
  const w = parseInt(el.style.width) || 42;
  const newW = Math.min(99, w + (Math.random() > 0.7 ? 1 : 0));
  el.style.width = newW + '%';
  const modalText = document.getElementById('cert-modal-text');
  if (modalText) modalText.textContent = newW + '%';
}
setInterval(() => advanceBar(), 3000);
